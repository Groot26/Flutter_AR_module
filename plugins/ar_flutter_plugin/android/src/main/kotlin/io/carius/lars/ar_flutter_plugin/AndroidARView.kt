...
internal class AndroidARView(
    ...
    // Cache the last plane hit results so addPlaneAnchor can use hitResult.createAnchor()
    // for a trackable-attached anchor, which is significantly more stable than a raw Pose anchor.
    private var lastPlaneHitResults: List<HitResult> = emptyList()
...
//                        onDestroy()
                        ...
                        .thenAccept { node ->
                            ...
                            if (anchorName != null && anchorType != null) {
                                ...
                                if (anchorNode != null) {
                                    ...
                                    // Reset to local origin so the model sits at the anchor
                                    // position. makeNodeFromGltf sets worldPosition=(0,0,0);
                                    // after reparenting Sceneform converts that to a non-zero
                                    // localPosition which places the model at world origin.
    ...
    private fun onTap(hitTestResult: HitTestResult, motionEvent: MotionEvent?): Boolean {
        ...
        if (motionEvent != null && motionEvent.action == MotionEvent.ACTION_DOWN) {
            // Fire plane/point tap whenever the user taps on empty space (no node hit).
            // This allows re-placement even when a node is currently selected.
            if (hitTestResult.node == null || (!enablePans && !enableRotation)) {
                ...
                // Cache plane hit results so addPlaneAnchor can attach to the trackable.
                lastPlaneHitResults = planeAndPointHitResults
    ...
    private fun addPlaneAnchor(transform: ArrayList<Double>, name: String): Boolean {
        return try {
            // Prefer a trackable-attached anchor: it follows the plane as ARCore refines its
            // world map, preventing models from drifting when you move the camera.
            val planeHit = lastPlaneHitResults.firstOrNull {
                it.trackable is Plane &&
                        (it.trackable as Plane).trackingState == TrackingState.TRACKING
            }
            val anchor: Anchor = if (planeHit != null) {
                lastPlaneHitResults = emptyList()  // consume the cache
                planeHit.createAnchor()
            } else {
                val t = deserializeMatrix4(transform)
                val position = floatArrayOf(t.second.x, t.second.y, t.second.z)
                val rotation = floatArrayOf(t.third.x, t.third.y, t.third.z, t.third.w)
                arSceneView.session!!.createAnchor(Pose(position, rotation))
            }
